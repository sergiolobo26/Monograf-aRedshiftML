...
def main():
    ...
    for i in range(n_repetitions):
        for j, n_data in enumerate(n_data_points):
            ...
            tuned_parameters = [{'kernel': ['rbf', 'poly'], 'gamma': [1e-3, 0.1, 1],
                     'C': [0.1, 1, 10]}]
            print("# Tuning hyper-parameters for %s" % score)
            print()
            #clf = GridSearchCV(KernelRidge(kernel='rbf', gamma=0.1), cv=3, n_jobs=n_jobs,
            #          param_grid={"alpha": [0.001, 0.0001],
            #                      "gamma": [0.1, 0.2], 'kernel':['rbf']}, refit=True)
            
            clf = GridSearchCV(SVR(), tuned_parameters, cv=3, n_jobs = n_jobs,
                       scoring='%s' % score, refit=True)
            ...
            model_name = 'svr_model_{}'.format(i)
            ...
            savetxt('{}_train.txt'.format(model_name), (y_train_pred, y_train, Z_tar_train))
            savetxt('{}_test.txt'.format(model_name), (y_pred, y_test, Z_tar_test))
